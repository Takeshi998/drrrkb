...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(prog="drrrkb", description="Unofficial Ring Racers Server Browser")
...
def updateServers():
    ...
    for addon in allServerAddons:
        ...
        # match prefixes
        if addonSplit[0].upper() == 'DC' or addonSplit[0].upper() == 'DCL':
            allServerAddonsSorted['Characters'][addon[0]] = addon[1]
        elif addonSplit[0].upper() == 'DR' or addonSplit[0].upper() == 'DRL' or addonSplit[0].upper() == 'DRB' or addonSplit[0].upper() == 'DB' or addonSplit[0].upper() == 'DBL' or addonSplit[0].upper() == 'DRBL':
            allServerAddonsSorted['Maps'][addon[0]] = addon[1]
        elif addonSplit[0].upper() == 'DL':
...
def appendServerInfo(ip='10.0.0.26', port='5029', contact=''):
    ...
    try:
        # hey hello what's up?
        serverInfo = drrrpacket.RR().Main(ip, int(port))
    ...
    if ip not in allServerFlags:
            ...
            # TODO: add a mechanic to retry connection after some time.
                ...
                allServerFlags[ip] = ['Unknown', 'XX', '', "XX"]
